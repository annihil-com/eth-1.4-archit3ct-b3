...
void unbanCommand() {
    ...
    if (system("mv ~/.etwolf/etmain/etkey ~/.etwolf/etmain/etkey.old") == -1)
      return;
...
void spamCommand() {
	...
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, "say \"^3  ___| |_| |__    *--------------*\"\n");
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, "say \"^3 / _ \\ __| '_ \\    ^3HACK INFOS :\"\n");

	char buf[512];
	memset(buf, 0, sizeof(buf));
	snprintf(buf, sizeof(buf), "say \"^3|  __/ |_| | | |  ^1 version^3[^7%s^3]\"\n", ETH_VERSION);

	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, buf);
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, "say \"^3 \\___|\\__|_| | |  ^1 website^3[^7ethcoders.free.fr^3]\"\n");
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, "say \"^3  by EthCoders /  *--------------*\"\n");
}

// version spam
void versCommand() {
	if (syscall_UI_Argc() != 1) {
		orig_syscall(UI_PRINT, "^nSpam with Eth version\n");
		orig_syscall(UI_PRINT, "^nUsage: eth_vers\n");
		return;
	}

	char buf[512];
	memset(buf, 0, sizeof(buf));
	snprintf(buf, sizeof(buf), "say \"^1version^3[^7%s^3]\"\n", ETH_VERSION);
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, buf);
}
...
void cleanPK3Command (void) {
	...
	if (system("find ~/.etwolf/ -name '*.pk3' -size -1200k -delete") == -1)
	  return;
...
void registerGameCommands() {
	...
	orig_Cmd_AddCommand(ETH_CMD_PREFIX "vers", &versCommand);
