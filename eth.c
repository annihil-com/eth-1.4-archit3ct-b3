...
char *Format(char *in){
	...
	for (a = 0, b = 0; a < len; a++) {
		if ((a <= len-2) && (in[a] == '[' && in[a+2] == ']')) {
			switch (in[a+1]) {
				case 'v':
				  strncat(out, eth.VictimName, sizeof(out) - strlen(out) - 1);
					break;
				case 'n':
					strncat(out, Q_CleanStr(eth.VictimName), sizeof(out) - strlen(out) - 1);
					break;
				case 't':
					strncat(out, va("%i", eth.killCount), sizeof(out) - strlen(out) - 1);
					break;
				case 'x':
					strncat(out, va("%i", eth.cg_snap->ps.stats[STAT_XP]), sizeof(out) - strlen(out) - 1);
					break;
				case 'q':
					strncat(out, va("%i", eth.pbss), sizeof(out) - strlen(out) - 1);
					break;					
				case 'p':
					strncat(out, eth.clientInfo[eth.cg_clientNum].name, sizeof(out) - strlen(out) - 1);
					break;
				case 'M':
				{
					char gameMod[20];
					syscall_CG_Cvar_VariableStringBuffer("fs_game", gameMod, sizeof(gameMod));
					strncat(out, gameMod, sizeof(out) - strlen(out) - 1);
					break;
				}
				case 'k':
					strncat(out, eth.KillerName, sizeof(out) - strlen(out) - 1);
					break;
				case 'K':
				{
					char *c = strdup(eth.KillerName);
					strncat(out, Q_CleanStr(c) , sizeof(out) - strlen(out) - 1);
					free(c);
					break;
				}
				case 'h':
					strncat(out, va("%i", eth.cg_snap->ps.stats[STAT_HEALTH]), sizeof(out) - strlen(out) - 1);
					break;								
				case 'c':
					strncat(out, va("%i", eth.killCountNoDeath), sizeof(out) - strlen(out) - 1);
					break;
					case '1': {
						char *multikillmsg ;
					...
					case '6': {
						...
						if (eth.killCount) {
							...
							break;
					strncat(out,killmsg, sizeof(out) - strlen(out) - 1);
						break;
					}
					case 'R': {
						char *msg;
					...
					case '5': {
						...
						if (eth.killSpreeCount == 2) {
							...
							break;
						strncat(out, multikillmsg, sizeof(out) - strlen(out) - 1);
						break;
					}
					case 'N': {
						char *multikillmsg;
