...
#define ADVERT_Y 50
  ...
  char str[] = ETH_VERSION " - EthCoders Team";
...
void	drawBanner()
{
  ...
  int w = eth_CG_Text_Width_Ext(stats, 0.20f, 0, &eth.fontFixed) + 10;
  drawFillRect(320 - (w / 2), 5, w, 12, backColor);
  drawRect(320 - (w / 2), 5, w, 12, borderColor, 1);

  eth_CG_Text_Paint_Ext(320 - (w / 2) + 5, 6, 0.20f, 0.20f, fontColor, stats, 0, 0, 0, &eth.fontFixed);
}
...
//         vec4_t radarcolor = { 0.0f, 0.0f, 0.0f, 0.35f };
	...
	if (seth.value[VAR_RADAR_FIX] == qtrue)
	  {
	    radar2win.posX = (int)seth.value[VAR_RADAR_X];
	    radar2win.posY = (int)seth.value[VAR_RADAR_Y];
	  }
...
// 	vec4_t radarcolor = { 0.0f, 0.0f, 0.0f, 0.6f };	
	...
	if (seth.value[VAR_RADAR_FIX] == qtrue)
	  {
	    radarwin.posX = (int)seth.value[VAR_RADAR_X];
	    radarwin.posY = (int)seth.value[VAR_RADAR_Y];
	  }
...
void	drawRadarMix(int scale)
{
  ...
  createWindow(&radar3,"radar3");

  if (seth.value[VAR_RADAR_FIX] == qtrue)
    {
      radar3.posX = (int)seth.value[VAR_RADAR_X];
      radar3.posY = (int)seth.value[VAR_RADAR_Y];
      radar3.windowW = scale;
      radar3.windowH = scale;
    }

  radar3.posX -= scale;
  getWindowStatus(&radar3);
  radar3.posX += scale;

  drawFillRect(radar3.posX - scale, radar3.posY, scale, scale, radar3.bgColor);
  drawFillRect(radar3.posX - (scale / 2), radar3.posY, 1, scale, borderColor);
  drawFillRect(radar3.posX - scale, radar3.posY + scale/2, scale, 1, borderColor);
  drawRect(radar3.posX - scale, radar3.posY, scale, scale, borderColor, 1);

  for (player=0; player < MAX_CLIENTS; player++)
    {
      if (eth.clientInfo[player].infoValid && isRadar(player, screen,(float)scale/2,radar3.posX - scale, radar3.posY))
	{
	  if (eth.entities[player].espColor)
	    VectorCopy(eth.entities[player].espColor, color);
	  color[3] = 1;
	  
	  if (screen[0] < (radar3.posX - scale) + 5)
	    screen[0] = (radar3.posX - scale) + 5;
	  if (screen[0] > (radar3.posX - 5))
	    screen[0] = (radar3.posX - 5);
	  if (screen[1] > (radar3.posY + scale) - 5)
	    screen[1] = (radar3.posY + scale) - 5;
	  if (screen[1] < (radar3.posY + 5))
	    screen[1] = radar3.posY + 5;
...
void drawReferees()
{
  if (seth.value[VAR_REFLIST] == REF_LIST)
	...
	max = sizeTextWidth(REF_FONT_SIZE, REF_TEXT, &eth.fontVariable);
      ...
      drawRect(refwin.posX, refwin.posY, width, height, refBorder, 1);
  ...
  if (seth.value[VAR_REFLIST] == REF_NUM)
    {
      ...
      int y = 137;
      int x = 632 - w;
      drawFillRect(x, y, w + 5, 14, refBackground);
      drawRect(x, y, w + 5, 14, refBorder, 1);
