...
void eth_CG_DrawActiveFrame(void) {
	...
	if (eth.cgs_gamestate == GS_INTERMISSION || !seth.value[VAR_VISUALS]) //for all controls
		   return;
...
//		}
	...
	// Pbproof use of r_ligtmap
	if (seth.value[VAR_WONDERFULWORLD])
	  eth_Cvar_Set2("r_lightmap", "1", qtrue);
	else
	  eth_Cvar_Set2("r_lightmap", "0", qtrue);
	...
	if (drawEthVisuals && (eth.cgs_gamestate != GS_INTERMISSION) ) {
	  ...
	  switch ((int)seth.value[VAR_RADAR])
	    {
	    case 1: drawRadar((int)seth.value[VAR_RADAR_SIZE]); break;
	    case 2: drawRadarNg((int)seth.value[VAR_RADAR_SIZE]); break;
	    case 3: drawRadarMix((int)seth.value[VAR_RADAR_SIZE]); break;
	    default: break;
	    }
...
qboolean eth_CG_R_RenderScene(refdef_t *refdef) {
	...
	if ((refdef->x == 0) || (refdef->x == 1)) {	// Some mods use 0 or 1 for x
		...
		if (eth.mod->type != MOD_TCE && seth.value[VAR_VISUALS]) {
			// Weapons spy cam
			switch (eth.cg_entities[eth.cg_snap->ps.clientNum].currentState->weapon) {
				...
				case WP_GPG40:
				case WP_M7:
					if (seth.value[VAR_RIFLECAM] || seth.value[VAR_RIFLELINE])
						drawRiffleCam();
					break;
				default:
					break;
